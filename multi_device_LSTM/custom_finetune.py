...
from custom_prediction import test_model
...
def predict_future(device_name, start_date, num_days, model_path, history_days=30):
...
def process_device(device_name, start_date, prediction_periods):
    ...
    for days in prediction_periods:
        ...
        results = predict_future(device_name, start_date, days, model_path, history_days=30)
        
        csv_filename = f'predictions_{device_name}_{days}days.csv'
        save_results(results, csv_filename)
        
        plot_filename = f'predictions_{device_name}_{days}days_plot.png'
        plot_title = f'{device_name} Load Prediction for Next {days} Days'
        plot_results(results, plot_title, plot_filename)

        # 计算并保存每日统计数据
        daily_stats = calculate_statistics(results)
        stats_filename = f'daily_stats_{device_name}_{days}days.csv'
...
if __name__ == "__main__":
    ...
    for csv_file in csv_files:
        ...
        process_device(device_name, start_date, prediction_periods)
